...
function applyHighlightFromQuery() {
  ...
  function ensureProductImages() {
    ...
    cards.forEach(function (card, index) {
      ...
      const imageSrc = manualImage || pool[index % pool.length];
      const existingImage = card.querySelector(".product-image");

      if (existingImage) {
        existingImage.setAttribute("src", imageSrc);
      ...
      imageWrapper.innerHTML =
        ...
        '<img class="product-image" src="' + imageSrc + '" alt="' + productTitle + '">' +
        "</div>";
  ...
  function ensureTenProducts() {
    ...
    const pageName = window.location.pathname.split("/").pop().toLowerCase();
    const targetCount = pageName === "nendoroid.html" ? 6 : 10;
